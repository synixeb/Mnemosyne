document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  async function refresh(){
    ...
    (res.personas||[]).forEach(p=>{
      ...
      inject.onclick = async ()=>{
        ...
        await chrome.tabs.sendMessage(tab.id, {action:'injectPersona', persona: p, username, aliasEmail: alias});
        window.close();
      };
  ...
  newBtn.onclick = async ()=>{
    const theme = prompt('Nom thématique pour la persona (ex: Jeux, Achats)', 'Persona');
